...
function writeeventstsv(EEG, fileOut, varargin)
...
%     % --- _events.tsv
    ...
    fid = fopen( fileOut, 'w');%[ fileOut '_events.tsv' ], 'w');
...
%     fprintf(fid,'onset%s\n', sprintf('\t%s', opt.eInfo{2:end,1}));
    ...
    % reorder fields to put required and default fields first
    fieldOrder = { 'latency' 'type'};
    newOrder = []; 
    for iField = 1:length(fieldOrder)
        ind = strmatch(fieldOrder{iField}, eventfields, 'exact');
        newOrder = [ newOrder ind ];
    end
    remainingInd = setdiff([1:length(eventfields)], newOrder);
    newOrder = [ newOrder remainingInd];
    eventfields= eventfields(newOrder,:);

    fprintf(fid,'%s%s\n', eventfields{1},sprintf('\t%s', eventfields{2:end}));
    % scan events
    for iEvent = 1:length(EEG.event)
        
        str = {};
        for iField = 1:numel(eventfields)
            tmpVal = EEG.event(iEvent).(eventfields{iField});
            if isnumeric(tmpVal)
